...
const TABS = ["Vue générale", "Document douanier", "Email source", "Historique"] as const;

function DossierPage() {
  const { reference } = useParams({ from: "/_espace/mains-levees/$reference" });
  const db = useDB();
  ...
  return (
    <>
      <div className="mb-4 flex items-center gap-2 text-[13px] text-muted-foreground">
        <Link to="/mains-levees" className="hover:text-foreground">
          Mains levées
        </Link>
              ...
              {declarant ? (
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    {session?.role === "ADMIN" ? (
                      <Btn variant="ghost" size="sm" className="mt-1" onClick={() => setEditOpen(true)}>
                ...
                <EmptyState
                  ...
                  action={
                    session?.role === "ADMIN" ? (
                      <Btn onClick={() => setEditOpen(true)}>Affecter un déclarant</Btn>
                    ) : undefined
                  }
                />
              )}
      ...
      {tab === "Historique" ? (
        <Surface title="Historique du dossier" description="Traçabilité complète des opérations.">
          <Timeline items={[...ml.history].reverse()} />
        </Surface>
      ) : null}
